function [json_parts, student_run_error] = terminalTest(student_run_error)
...
while AUTOGRADER_INDEX <= length(AUTOGRADER_INPUTS)
  ...
  terminalValues = AUTOGRADER_INPUTS{AUTOGRADER_INDEX}{1};
  testName = sprintf("Terminal Test with input vlaues: [ %s ]",terminalValues);
  try
	warning('off', 'all');
	...
	if old_index == AUTOGRADER_INDEX
		...
		json_parts{end+1} = build_json_test(testName, sum(terminal_test_points), 0, output_msg, 'visible')
    return;
	end
  catch ME
  ...
  if (student_run_error)
    ...
    if ~strcmp(student_output, reference_output)
      ...
      if isempty(diff_idx) % Strings are identical up to the min length, one is longer
        ...
        reference_output(max(1, diff_idx-5):min(end, diff_idx+5)), ...
        student_output(max(1, diff_idx-5):min(end, diff_idx+5)));
      end
      %error(feedback); % This will fail the test and show the feedback
      score = 0;

      json_parts{end+1} = build_json_test(testName, terminal_test_points, score, feedback, 'visible');

    else
      feedback = sprintf('Output matches!'); % Or simply let the test pass
      json_parts{end+1} = build_json_test(testName, terminal_test_points, terminal_test_points, feedback, 'visible');
    end
